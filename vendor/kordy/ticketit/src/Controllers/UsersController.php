<?php

namespace Kordy\Ticketit\Controllers;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Session;
use Kordy\Ticketit\Models\Agent;
use Kordy\Ticketit\Models\Setting;

class UsersController extends Controller
{
    public function index()
    {
        $users = Agent::where('role', 'patient')->get();

        return view('ticketit::admin.user.index', compact('users'));
    }
    
    public function edit($id)
    {
        $user = Agent::where('id', $id)->first();

        return view('ticketit::admin.user.edit', compact('user'));
    }

    public function create()
    {
        //$users = Agent::where('ticketit_agent', 1)->paginate(Setting::grab('paginate_items'));
        //$categories = \Kordy\Ticketit\Models\Category::where('Name', 'NOT LIKE', 'Unidad%')->get();
        //$units = \Kordy\Ticketit\Models\Category::where('Name', 'LIKE', 'Unidad%')->get();
        return view('ticketit::admin.user.create');
    }

    public function store(Request $request)
    {
        $agents_list = $this->addAgents($request->input('agents'));
        $agents_names = implode(',', $agents_list);

        Session::flash('status', trans('ticketit::lang.agents-are-added-to-agents', ['names' => $agents_names]));

        return redirect()->action('\Kordy\Ticketit\Controllers\AgentsController@index');
    }

    public function update($id, Request $request)
    {
        $this->syncAgentCategories($id, $request);

        Session::flash('status', trans('ticketit::lang.agents-joined-categories-ok'));

        return redirect()->action('\Kordy\Ticketit\Controllers\AgentsController@index');
    }

    public function destroy($id)
    {
        $user = Agent::find($id);
        $user_name = $user->name.' '.$user->l_name_1.' '.$user->l_name_2;
        
        if ($this->removeUser($id)) {
            Session::flash('status', trans('ticketit::lang.user-is-removed-from-team', ['name' => $user_name]));
            return redirect()->action('\Kordy\Ticketit\Controllers\UsersController@index');
        }
    }

    /**
     * Assign users as agents.
     *
     * @param $user_ids
     *
     * @return array
     */
    public function addAgents($user_ids)
    {
        $users = Agent::find($user_ids);
        foreach ($users as $user) {
            $user->ticketit_agent = true;
            $user->save();
            $users_list[] = $user->name;
        }

        return $users_list;
    }

    /**
     * Remove user from the agents.
     *
     * @param $id
     *
     * @return mixed
     */
    public function removeAgent($id)
    {
        $agent = Agent::find($id);
        $agent->ticketit_agent = false;
        $agent->save();

        // Remove him from tickets categories as well
        if (version_compare(app()->version(), '5.2.0', '>=')) {
            $agent_cats = $agent->categories->pluck('id')->toArray();
        } else { // if Laravel 5.1
            $agent_cats = $agent->categories->lists('id')->toArray();
        }

        $agent->categories()->detach($agent_cats);

        return $agent;
    }
    
     /**
     * Remove user from the agents.
     *
     * @param $id
     *
     * @return mixed
     */
    public function removeUser($id)
    {
        $user = Agent::find($id);
        $user->delete();
        
        //$user_cats = $user->categories->pluck('id')->toArray();        

        //$user->categories()->detach($user_cats);

        return true;
    }

    /**
     * Sync Agent categories with the selected categories got from update form.
     *
     * @param $id
     * @param Request $request
     */
    public function syncAgentCategories($id, Request $request)
    {
        $form_cats = ($request->input('agent_cats') == null) ? [] : $request->input('agent_cats');
        $agent = Agent::find($id);
        $agent->categories()->sync($form_cats);
    }
}
